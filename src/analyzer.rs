...
use crate::{
    ...
    packet::{try_read_packet, RawPacket},
};
...
impl Analyzer {
        ...
        match self.phase {
            Phase::ServerHello => {
                ...
                self.emit(Event::ServerHello {
                    ...
                    auth_plugin:    hs.auth_plugin_name_str()
                                      .map(|c| c.into_owned())
                                      .unwrap_or_default(),
                });
                self.phase = Phase::ClientResponse { server_caps: hs.capabilities() };
            }
    ...
    fn on_server_command_response(
        ...
        if first == 0x00 && pkt.payload.len() == 12 {
            if let Pending::StmtPrepare(ref sql) = self.pending {
                ...
                self.stmts.insert(stmt_id, PreparedStmt {
                    sql:        sql.clone(),
                    num_params,
                    last_types: None,
                });
    ...
    fn handle_command(&mut self, pkt: &RawPacket, caps: CapabilityFlags) {
        let Some(&cmd_byte) = pkt.payload.first() else { return };
        ...
        if cmd_byte == Command::COM_QUERY as u8 {
            ...
            self.emit(Event::Query { sql: lossy_trim(sql_bytes) });
...
fn parse_execute_params(
    ...
    if num_params == 0 {
        return Some(ParsedParams { types: vec![], values: vec![] });
    }
    ...
    let bmap_len = (n + 7) / 8;
    if rest.len() < bmap_len + 1 { return None; }
    ...
    let types: Vec<(ColumnType, ColumnFlags)> = if new_params_bound == 1 {
        ...
        for _ in 0..n {
            if buf.0.len() < 2 { return None; }
            let col_type = ColumnType::try_from(buf.0[0])
...
fn parse_query_attrs_prefix(after_cmd: &[u8]) -> &[u8] {
    let Some((&param_count, rest)) = after_cmd.split_first() else { return after_cmd };
    if param_count >= 0xFB { return after_cmd; }
    let Some((_, rest2)) = rest.split_first() else { return after_cmd };
    if param_count == 0 { return rest2; }
    after_cmd
}
